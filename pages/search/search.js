  ...
  getdriverPublish: async function() {
    ...
    wx.request({
      ...
      success: (res)=> {
        ...
        if( _this.data.msgList.length != res.data.queryResult.rows.length){
          _this.setData({
            msgList: _this.data.msgList.concat(res.data.queryResult.rows),
            allowload: true,
            msglistLength: res.data.queryResult.count
          })
        }
